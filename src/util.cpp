...
bool saveBMP(const std::string &filename, const BMPImage &img) {
    ...
    fileHeader.size = static_cast<std::uint32_t>(fileHeader.offset + pixelDataSize);
    ...
    for (std::int32_t y = 0; y < absHeight; ++y) {
    // Khi ghi ảnh, cần đảo ngược thứ tự hàng nếu chiều cao dương để phù hợp với chuẩn BMP bottom-up.
...
// Tạo tên file đầu ra bằng cách chèn chuỗi "_processed" trước phần mở rộng
...
// Tính số byte padding cần thêm để mỗi hàng dữ liệu pixel có độ dài chia hết cho 4
